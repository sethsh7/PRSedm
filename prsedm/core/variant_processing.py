...
def get_af_rec(df):
    ...
    if 'FORMAT' in df.columns and any(
            col.startswith('GT') for col in df.columns):
        allele_counts = [0, 0]
        ...
        if total_alleles > 0:
            ...
            raise ValueError(
                "Error: Unable to calculate AF; no valid allele data found.")
    raise ValueError(
        "Error: No AF in INFO column and no GT in VCF to calculate AF.")
